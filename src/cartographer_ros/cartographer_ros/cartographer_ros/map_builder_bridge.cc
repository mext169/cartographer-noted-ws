...
visualization_msgs::Marker CreateLandmarkMarker(int landmark_index,
  ...
  marker.type = visualization_msgs::Marker::SPHERE; // http://wiki.ros.org/rviz/DisplayTypes/Marker
  marker.header.stamp = ::ros::Time::now();
  marker.header.frame_id = frame_id;
...
void MapBuilderBridge::LoadState(const std::string& state_filename,
  ...
  // Check if suffix(后缀) of the state file is ".pbstream".
 ...
 */
void MapBuilderBridge::HandleSubmapQuery(cartographer_ros_msgs::SubmapQuery::Request& request, cartographer_ros_msgs::SubmapQuery::Response& response) {
...
visualization_msgs::MarkerArray MapBuilderBridge::GetTrajectoryNodeList() {
  ...
  for (const int trajectory_id : node_poses.trajectory_ids()) {
    ...
    int last_inter_submap_constrained_node = std::max(node_poses.trajectory(trajectory_id).begin()->id.node_index,
        trajectory_to_last_inter_submap_constrained_node.at(trajectory_id));
    int last_inter_trajectory_constrained_node = std::max(node_poses.trajectory(trajectory_id).begin()->id.node_index,
        trajectory_to_last_inter_trajectory_constrained_node.at(trajectory_id));
    
    // 节点索引的最大值
    last_inter_submap_constrained_node = std::max(last_inter_submap_constrained_node, last_inter_trajectory_constrained_node);
