 ...
 * @return true: ROS的service只能返回true, 返回false程序会中断
 */
bool Node::HandleTrajectoryQuery(::cartographer_ros_msgs::TrajectoryQuery::Request& request, ::cartographer_ros_msgs::TrajectoryQuery::Response& response) {
  absl::MutexLock lock(&mutex_);
...
/**
 * @brief 新增一个位姿估计器，IMU和里程计的融合，作为前端的先验
 * 
 * @param[in] trajectory_id 轨迹id
...
void Node::PublishLocalTrajectoryData(const ::ros::TimerEvent& timer_event) {
  ...
  for (const auto& entry : map_builder_bridge_.GetLocalTrajectoryData()) {
    ...
    if (last_published_tf_stamps_.count(entry.first) && last_published_tf_stamps_[entry.first] == stamped_transform.header.stamp)
      continue;
