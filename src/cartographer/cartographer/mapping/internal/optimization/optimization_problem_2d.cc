...
transform::Rigid3d GetInitialLandmarkPose(
  ...
  const double interpolation_parameter = common::ToSeconds(observation.time - prev_node.time) / common::ToSeconds(next_node.time - prev_node.time);
  // 根据landmark数据的时间对2个节点位姿进行插值, 得到这个时刻的tracking_frame在global坐标系下的位姿
  const std::tuple<std::array<double, 4>, std::array<double, 3>>
      rotation_and_translation = InterpolateNodes2D(prev_node_pose.data(), prev_node.gravity_alignment,
                                                    next_node_pose.data(), next_node.gravity_alignment,
                                                    interpolation_parameter);
  // 将landmark的数据从tracking_frame下的位姿转到global坐标系下
  // cpp123 https://en.cppreference.com/w/cpp/utility/tuple/get
  return transform::Rigid3d::FromArrays(std::get<0>(rotation_and_translation),
                                        std::get<1>(rotation_and_translation)) * observation.landmark_to_tracking_transform;
}
  ...
  for (const auto& landmark_node : landmark_nodes) {
    for (const auto& observation : landmark_node.second.landmark_observations) {
      ...
      if (!C_landmarks->count(landmark_id)) {
        ...
        const transform::Rigid3d starting_point = landmark_node.second.global_landmark_pose.has_value()
...
// 添加gps数据
void OptimizationProblem2D::AddFixedFramePoseData(const int trajectory_id, const sensor::FixedFramePoseData& fixed_frame_pose_data) {
...
void OptimizationProblem2D::Solve(const std::vector<Constraint>& constraints,
  ...
  for (auto node_it = node_data_.begin(); node_it != node_data_.end();) {
    ...
    for (++node_it; node_it != trajectory_end; ++node_it) {
      ...
      std::unique_ptr<transform::Rigid3d> relative_odometry = CalculateOdometryBetweenNodes(trajectory_id,
                                                                                            first_node_data,
                                                                                            second_node_data);
      ...
      if (relative_odometry != nullptr) {
        problem.AddResidualBlock(
            CreateAutoDiffSpaCostFunction(Constraint::Pose{*relative_odometry,
      ...
      const transform::Rigid3d relative_local_slam_pose = transform::Embed3D(first_node_data.local_pose_2d.inverse() * second_node_data.local_pose_2d);
  ...
  for (auto node_it = node_data_.begin(); node_it != node_data_.end();) {
    ...
    for (; node_it != trajectory_end; ++node_it) {
      ...
      const std::unique_ptr<transform::Rigid3d> fixed_frame_pose = Interpolate(fixed_frame_pose_data_, trajectory_id, node_data.time);
...
std::unique_ptr<transform::Rigid3d> OptimizationProblem2D::InterpolateOdometry(const int trajectory_id, const common::Time time) const {
...
OptimizationProblem2D::CalculateOdometryBetweenNodes(const int trajectory_id, const NodeSpec2D& first_node_data,
