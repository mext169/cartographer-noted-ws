...
LocalTrajectoryBuilder2D::TransformToGravityAlignedFrameAndFilter(
  ...
  const sensor::RangeData cropped = sensor::CropRangeData(sensor::TransformRangeData(range_data, transform_to_gravity_aligned_frame),
...
LocalTrajectoryBuilder2D::AddRangeData(const std::string& sensor_id, const sensor::TimedPointCloudData& unsynchronized_data) {
  ...
  // param: num_accumulated_range_data 累积几帧有效的点云数据之后就进行一次扫描匹配
...
LocalTrajectoryBuilder2D::AddAccumulatedRangeData(
  ...
  const sensor::PointCloud& filtered_gravity_aligned_point_cloud = sensor::AdaptiveVoxelFilter(gravity_aligned_range_data.returns,
  ...
  // 校准位姿估计器
  // 位姿估计器给扫描匹配提供初始位姿，扫描匹配输出的位姿又传给位姿估计器
  ...
  // 将校正后的雷达数据写入submap
  std::unique_ptr<InsertionResult> insertion_result = InsertIntoSubmap(time, range_data_in_local, filtered_gravity_aligned_point_cloud,
  ...
  // 返回结果 
  return absl::make_unique<MatchingResult>(MatchingResult{time, pose_estimate, std::move(range_data_in_local), std::move(insertion_result)});
}
...
void LocalTrajectoryBuilder2D::InitializeExtrapolator(const common::Time time) {
  ...
  extrapolator_ = absl::make_unique<PoseExtrapolator>(
      ::cartographer::common::FromSeconds(options_.pose_extrapolator_options().constant_velocity().pose_queue_duration()), // 0.001s
