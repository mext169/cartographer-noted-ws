...
std::vector<SubmapId> PoseGraph2D::InitializeGlobalSubmapPoses(const int trajectory_id,
...
void PoseGraph2D::AddTrajectoryIfNeeded(const int trajectory_id) {
  ...
  if (!global_localization_samplers_[trajectory_id]) {
    global_localization_samplers_[trajectory_id] = absl::make_unique<common::FixedRatioSampler>(options_.global_sampling_ratio());
  }
}
...
void PoseGraph2D::ComputeConstraint(const NodeId& node_id,
...

  {
    ...
    // 两个轨迹的最后连接时间
    const common::Time last_connection_time = data_.trajectory_connectivity_state.LastConnectionTime(node_id.trajectory_id,
 ...
 * @param[in] insertion_submaps active_submaps 当前维护的两个子图
...
WorkItem::Result PoseGraph2D::ComputeConstraintsForNode(const NodeId& node_id,
  ...
  // 保存节点与计算子图内约束
  {
    ...
    const transform::Rigid2d local_pose_2d = transform::Project2D(constant_data->local_pose * // 三维转平面
  ...
  if (global_slam_optimization_callback_) {
    ...
    std::map<int, SubmapId> trajectory_id_to_last_optimized_submap_id;
    {
      ...
      for (const int trajectory_id : node_data.trajectory_ids()) {
        ...
        trajectory_id_to_last_optimized_node_id.emplace(trajectory_id, std::prev(node_data.EndOfTrajectory(trajectory_id))->id);
        trajectory_id_to_last_optimized_submap_id.emplace(trajectory_id, std::prev(submap_data.EndOfTrajectory(trajectory_id))->id);
      }
    }
...
// 计算 global frame 指向 local frame 的坐标变换
transform::Rigid3d PoseGraph2D::ComputeLocalToGlobalTransform(const MapById<SubmapId, optimization::SubmapSpec2D>& global_submap_poses,
