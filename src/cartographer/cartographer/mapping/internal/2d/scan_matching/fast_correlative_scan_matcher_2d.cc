...
PrecomputationGridStack2D::PrecomputationGridStack2D(const Grid2D& grid,
  ...
  for (int i = 0; i != options.branch_and_bound_depth(); ++i) {
    ...
    // cpp123 把这些散件放在一起emplace_back后，就直接构造了
...
bool FastCorrelativeScanMatcher2D::MatchFullSubmap(
  ...
  const transform::Rigid2d center = transform::Rigid2d::Translation(
      limits_.max() - 0.5 * limits_.resolution() * Eigen::Vector2d(limits_.cell_limits().num_y_cells, limits_.cell_limits().num_x_cells));
...
bool FastCorrelativeScanMatcher2D::MatchWithSearchParameters(
  ...
  const sensor::PointCloud rotated_point_cloud = sensor::TransformPointCloud(
      point_cloud, transform::Rigid3f::Rotation(Eigen::AngleAxisf(initial_rotation.cast<float>().angle(), Eigen::Vector3f::UnitZ())));
  ...
  const std::vector<Candidate2D> lowest_resolution_candidates = ComputeLowestResolutionCandidates(discrete_scans, search_parameters);
...
std::vector<Candidate2D> FastCorrelativeScanMatcher2D::ComputeLowestResolutionCandidates(const std::vector<DiscreteScan2D>& discrete_scans,
...
// 对所有的候选解进行评分并进行降序排序
void FastCorrelativeScanMatcher2D::ScoreCandidates(const PrecomputationGrid2D& precomputation_grid, const std::vector<DiscreteScan2D>& discrete_scans,
                                                   const SearchParameters& search_parameters, std::vector<Candidate2D>* const candidates) const {
  // 遍历所有的候选解, 对每个候选解进行打分
  for (Candidate2D& candidate : *candidates) {
    ...
    candidate.score = precomputation_grid.ToScore(sum / static_cast<float>(discrete_scans[candidate.scan_index].size()));
  }
