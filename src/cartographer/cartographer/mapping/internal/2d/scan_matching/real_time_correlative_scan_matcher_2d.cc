...
std::vector<Candidate2D> RealTimeCorrelativeScanMatcher2D::GenerateExhaustiveSearchCandidates(
  ...
  for (int scan_index = 0; scan_index != search_parameters.num_scans; ++scan_index) {
    const int num_linear_x_candidates = (search_parameters.linear_bounds[scan_index].max_x -
                                        search_parameters.linear_bounds[scan_index].min_x + 1);
    const int num_linear_y_candidates = (search_parameters.linear_bounds[scan_index].max_y -
...
double RealTimeCorrelativeScanMatcher2D::Match(
  ...
  // 一共 search_parameters.num_scans 个点云，旋转角度不同
...
void RealTimeCorrelativeScanMatcher2D::ScoreCandidates(const Grid2D& grid, const std::vector<DiscreteScan2D>& discrete_scans,
  ...
  // num_scans * num_linear_x_candidates * num_linear_y_candidates 次循环
  // num_scans 组不同旋转角度的激光雷达帧
  // 对于没一组特征角度的激光雷达帧，又有num_linear_x_candidates * num_linear_y_candidates组不同的偏移位置
  for (Candidate2D& candidate : *candidates) {
    ...
    // 对得分进行加权
    // 偏移越大，对应的权重也就越小
    candidate.score *= std::exp(-common::Pow2(std::hypot(candidate.x, candidate.y) * options_.translation_delta_cost_weight() +
