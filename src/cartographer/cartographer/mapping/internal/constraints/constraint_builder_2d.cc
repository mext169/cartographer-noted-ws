...
void ConstraintBuilder2D::MaybeAddConstraint(
  ...
  // 超过范围的不进行约束的计算
  if (initial_relative_pose.translation().norm() > options_.max_constraint_distance()) { // param: max_constraint_distance
    return;
  }
  // 根据参数配置添加约束的频率
  // cpp123 .emplace返回一个pair，pair的第一个是指向插入元素的迭代器，pair的第二个为true或false，表示实际是否插入成功了
  if (!per_submap_sampler_.emplace(std::piecewise_construct, 
                                   std::forward_as_tuple(submap_id), 
                                   std::forward_as_tuple(options_.sampling_ratio())).first->second.Pulse()) {
    return;
  }
  ...
  // 为子图新建一个匹配器
  // DispatchScanMatcherConstruction中将任务放到了线程池中运行 因为构造地图匹配器的时候要生成多分辨率地图 比较耗时
...
void ConstraintBuilder2D::WhenDone(const std::function<void(const ConstraintBuilder2D::Result&)>& callback) {
  ...
  // 生成 执行when_done_的任务
  // when_done_task_ 变成了 work_item_
...
void ConstraintBuilder2D::ComputeConstraint(
  ...
  if (match_full_submap) {
    ...
    if (submap_scan_matcher.fast_correlative_scan_matcher->MatchFullSubmap(constant_data->filtered_gravity_aligned_point_cloud,
  ...
  ceres_scan_matcher_.Match(pose_estimate.translation(),
                            pose_estimate,
                            constant_data->filtered_gravity_aligned_point_cloud,
