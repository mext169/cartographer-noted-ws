...
class MapLimits {
 public:
  ...
  Eigen::Array2i GetCellIndex(const Eigen::Vector2f& point) const {
    ...
    // rotate.
    // max.x()和max.y()左上角 因此这里获得的是相对于地图左上角的坐标（cartographerd的像素坐标系原点在左上角）
  ...
  // 根据像素索引算物理坐标 (cartogapher的地图坐标系是以右下角为原点的)
 ...
 private:
  ...
  // 这是一个浮点型二维向量，max_.x()和.y()分别表示x、y方向的最大值
  // 根据最大范围max_和分辨率resolution就可以创建cell_limits
