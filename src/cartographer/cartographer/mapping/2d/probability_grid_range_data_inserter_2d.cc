...
void GrowAsNeeded(const sensor::RangeData& range_data, ProbabilityGrid* const probability_grid) {
  // 找到点云的bounding_box
  Eigen::AlignedBox2f bounding_box(range_data.origin.head<2>()); // .head<n>提取前n个元素
...
void CastRays(const sensor::RangeData& range_data,
  ...
  // 根据雷达数据调整地图范围 根据点云的bounding box, 看是否需要对地图进行扩张
  GrowAsNeeded(range_data, probability_grid);

  // 获取栅格地图的limits
  const MapLimits& limits = probability_grid->limits();
  // 定义一个超分辨率像素，把当前的分辨率又划分成了kSubpixelScale（1000）份
  ...
  for (const sensor::RangefinderPoint& hit : range_data.returns) {
    ...
    // 更新hit点的栅格值
    // ends.back() / kSubpixelScale就变成了一般分辨率下的坐标了
  ...
  for (const sensor::RangefinderPoint& missing_echo : range_data.misses) {
    std::vector<Eigen::Array2i> ray = RayToPixelMask(begin, superscaled_limits.GetCellIndex(missing_echo.position.head<2>()), kSubpixelScale);
    for (const Eigen::Array2i& cell_index : ray) {
...
ProbabilityGridRangeDataInserter2D::ProbabilityGridRangeDataInserter2D(
    ...
    : options_(options),
      ...
      hit_table_(ComputeLookupTableToApplyCorrespondenceCostOdds(Odds(options.hit_probability()))),    // 0.55
      // 生成更新空闲栅格时的查找表 // param: miss_probability
      miss_table_(ComputeLookupTableToApplyCorrespondenceCostOdds(Odds(options.miss_probability()))) {} // 0.49
...
void ProbabilityGridRangeDataInserter2D::Insert(const sensor::RangeData& range_data, GridInterface* const grid) const {
  // 类型强制转换 GridInterface* -> ProbabilityGrid*
