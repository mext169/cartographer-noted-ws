...
void ImuTracker::Advance(const common::Time time) {
  ...
  // 要预测的时间要比ImuTracker维护的时间晚
  CHECK_LE(time_, time);
  const double delta_t = common::ToSeconds(time - time_);
  // 上一时刻的角速度乘以时间,得到当前时刻相对于上一时刻的预测的姿态变化量,再转换成四元数
  const Eigen::Quaterniond rotation =transform::AngleAxisVectorToRotationQuaternion(Eigen::Vector3d(imu_angular_velocity_ * delta_t));
  // 使用上一时刻的姿态 orientation_ 乘以姿态变化量, 得到当前时刻的预测出的姿态
  // Note: 这里是右乘 Ref: https://blog.csdn.net/weixin_45632220/article/details/117735223
  // 右乘是要变换回去，得到的是当前的姿态在初始坐标系下的表示
  orientation_ = (orientation_ * rotation).normalized();

  // 根据预测出的姿态变化量,预测旋转后的线性加速度的值
  // 变换回去，得到加速度方向在初始坐标系下的表示
...
void ImuTracker::AddImuLinearAccelerationObservation(const Eigen::Vector3d& imu_linear_acceleration) {
  ...
  // delta_t越大，alpha越大，就舍弃更多的原来的数据gravity_vector_，使用更多的当前的数据imu_linear_acceleration
  ...
  // orientation_.conjugate() * Eigen::Vector3d::UnitZ() 得到当前姿态在初始坐标系下的表示
  // TODO 细细思考一下这里
