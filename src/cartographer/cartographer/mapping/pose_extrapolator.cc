...
// LocalTrajectoryBuilder2D::AddAccumulatedRangeData中调用
void PoseExtrapolator::AddPose(const common::Time time, const transform::Rigid3d& pose) {
  ...
  // 因为pose是扫描匹配的结果 之前的imu及里程计数据是用于预测的,现在结果都有了,之前的用于预测的数据肯定不需要了
  TrimImuData();
  TrimOdometryData();

  // 用于根据里程计数据计算线速度时姿态的预测
  // 更新pose后，重新生成odometry_imu_tracker_和extrapolation_imu_tracker_
...
/**
 * @brief 取出timed_pose_queue_这个队列中最早和最新的两个Pose做差，然后除以时间得到机器人的速度。但是最早和最新的两个时间差要满足要求，间隔不能太短
 * 
 */
...
void PoseExtrapolator::AdvanceImuTracker(const common::Time time, ImuTracker* const imu_tracker) const {
  // 检查指定时间是否大于等于 ImuTracker 的时间，如果是大于的，那么需要更新ImuTracker
  CHECK_GE(time, imu_tracker->time());

  // 不使用imu 或者 预测时间之前没有imu数据的情况
  // 如果IMU数据队列为空，或者当前时间比IMU数据队列中最早期的时间还要早。则说明没有可用的IMU数据
  // 这时候根据时间推算。对于角速度，使用从pose中估计的角速度或从里程计获得的角速度来更新ImuTracker角速度
...
PoseExtrapolator::ExtrapolatePosesWithGravity(const std::vector<common::Time>& times) {
  ...
  return ExtrapolationResult{poses, ExtrapolatePose(times.back()), current_velocity, EstimateGravityOrientation(times.back())};
}
