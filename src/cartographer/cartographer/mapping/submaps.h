...
// Submap定义在/mapping/submaps.h中，这是一个接口类，根据2d和3d的情况不同，
// 分别有Submap2D (/mapping/2d/submap_2d.h)和Submap3D (/mapping/3d/submap_3d.h)两种实现方式。
// 一个子图，首先要有一个local_pose，这个local_pose可以看做是没有经过全局优化的该submap相对于世界坐标系的位姿
// 一张子图要不停地监视有多少个range data（这个range data我理解为点云）被铲入到这个submap当中.
