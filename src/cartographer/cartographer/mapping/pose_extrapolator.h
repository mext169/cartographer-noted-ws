...
class PoseExtrapolator : public PoseExtrapolatorInterface {
 ...
 private:
  ...
  // 带时间的pose队列
  // 维护着pose_queue_duration这段时间内的位姿
  std::deque<TimedPose> timed_pose_queue_; // 来自扫描匹配的位姿
  // 线速度有2种计算途径
  // 从持续跟踪一段时间的Pose队列中估计出来的线速度和角速度，初始化为0
  Eigen::Vector3d linear_velocity_from_poses_ = Eigen::Vector3d::Zero();
  Eigen::Vector3d angular_velocity_from_poses_ = Eigen::Vector3d::Zero();

  const double gravity_time_constant_;
  std::deque<sensor::ImuData> imu_data_; // imu数据
  ...
  std::unique_ptr<ImuTracker> imu_tracker_;               // 保存与预测当前姿态 只在添加scanmatch位姿时更新 用于保存添加校准位姿时的姿态
