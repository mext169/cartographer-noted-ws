...
int MapBuilder::AddTrajectoryBuilder(const std::set<SensorId>& expected_sensor_ids,
  ...
  if (options_.use_trajectory_builder_3d()) {
    ...
    // LocalTrajectoryBuilder3D这个类没有继承TrajectoryBuilder，只是一个工具类
    //   LocalTrajectoryBuilder3D定义在/mapping/internal/3d/local_trajectory_builder_3d.h中
  ...
  // 2d的轨迹
      ...
      // LocalTrajectoryBuilder2D这个类没有继承TrajectoryBuilder，只是一个工具类
...
int MapBuilder::AddTrajectoryForDeserialization(const proto::TrajectoryBuilderOptionsWithSensorIds& options_with_sensor_ids_proto) {
  const int trajectory_id = trajectory_builders_.size();

  // c++11: vector::emplace_back() 在原地构造, 直接传入vector, 不调用移动构造函数
  trajectory_builders_.emplace_back(); // TODO 为什么这里emplace_back了个空空的内容
  all_trajectory_builder_options_.push_back(options_with_sensor_ids_proto);
  CHECK_EQ(trajectory_builders_.size(), all_trajectory_builder_options_.size());
  return trajectory_id;
}
...
// 分别调用sensor_collator和pose_graph_的成员函数来Finish一条轨迹
void MapBuilder::FinishTrajectory(const int trajectory_id) {
    // 这个Finish应该是和清楚传感器的占用有关
  sensor_collator_->FinishTrajectory(trajectory_id);
  // 这个Finish应该是和trajectory的全局优化有关
  pose_graph_->FinishTrajectory(trajectory_id);
}

// 返回压缩后的地图数据
// 根据指定的submap_id来查询submap，把结果放在response里面
...
// 将数据进行压缩,并保存到文件中
// map_builder_bridge调用，更上层在Node中调用
...
std::map<int, int> MapBuilder::LoadState(io::ProtoStreamReaderInterface* const reader, bool load_frozen_state) {
  ...
  for (auto& constraint_proto : *pose_graph_proto.mutable_constraint()) {
    constraint_proto.mutable_submap_id()->set_trajectory_id(trajectory_remapping.at(constraint_proto.submap_id().trajectory_id()));
    constraint_proto.mutable_node_id()->set_trajectory_id(trajectory_remapping.at(constraint_proto.node_id().trajectory_id()));
  }
